...
def visualize(training_data, X, y, scaler, pca, features):
    ...
    cmap = sb.blend_palette(["#00008B", "#6A5ACD", "#F0F8FF", "#FFE6F8", "#C71585", "#8B0000"], as_cmap=True)
    sb.corrplot(training_data, annot=False, sig_stars=False, diag_names=False, cmap=cmap, ax=ax2)
    fig2.tight_layout()
...
def train(X, y, algorithm, scaler, pca):
    """
...
def main():
    ...
    if train_model:
        ...
        model = train(X, y, algorithm, scaler, pca)
